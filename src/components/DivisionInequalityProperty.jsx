...
const DivisionInequalityProperty = () => {
  ...
  const generateProblem = () => {
    ...
    let coefficient = Math.floor(Math.random() * 11) + 2;
    // Generate random solution (-9 to 9, excluding 0)
    let solution = Math.floor(Math.random() * 19) - 9;
    while (solution === 0) {
      solution = Math.floor(Math.random() * 19) - 9;
    }
    ...
    const intermediateSteps = {
      step1: `${coefficient}x ÷ (${coefficient}) ${inequalitySign} ${result} ÷ (${coefficient})`,
      step2: `x ${coefficient < 0 ? (inequalitySign === '>' ? '<' : '>') : inequalitySign} ${solution}`
    };

    return {
      equation: `${coefficient}x ${inequalitySign} ${result}`,
      coefficient,
      result,
      inequalitySign,
  ...
  const checkAnswer = (step, answer) => {
    ...
    if (step === 1) {
      ...
      const normalizedExpected = normalizeEquation(currentProblem.intermediateSteps.step1);
      ...
      const normalizedInput = normalizeEquation(answer);
      const normalizedExpected = normalizeEquation(currentProblem.intermediateSteps.step2);
  ...
  const skipStep = (step) => {
    if (step === 1) {
      setStep1Answer(currentProblem.intermediateSteps.step1);
      setCompletedSteps(prev => ({ ...prev, step1: true }));
      setStepSkipped(prev => ({ ...prev, step1: true }));
    } else if (step === 2) {
      setStep2Answer(currentProblem.intermediateSteps.step2);
      setCompletedSteps(prev => ({ ...prev, step2: true }));
      ...
      <div className="w-[500px] h-auto mx-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-2px_rgba(0,0,0,0.1),0_0_0_1px_rgba(0,0,0,0.05)] bg-white rounded-lg overflow-hidden">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-[#5750E3] text-sm font-medium select-none">Division Property of Inequality</h2>
            <Button 
          ...
          <div className="bg-gray-50">
            <div className="p-4 space-y-4">
              <div className="w-full p-2 mb-1 bg-white border border-[#5750E3]/30 rounded-md">
                {currentStepIndex === 0 && (
                  <>
                    <p className="text-sm mb-2 text-left">Step 1: Divide both sides by the coefficient</p>
                ...
                {currentStepIndex === 1 && completedSteps.step1 && (
                  <>
                    <p className="text-sm mb-2 text-left">Step 2: Write the final answer</p>
                    <div className="space-y-2">
